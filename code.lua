do
    l
ocal b = {}
    for i = 0, 255 do b[i] = string.char(i) end
    ...
    local encoded = "MjAyNS0wNi0yNQ=="
    local decoded = base64decode(encoded)
    local year, month, day = decoded:match("(%d+)%-(%d+)%-(%d+)")
    year, month, day = tonumber(year), tonumber(month), tonumber(day)

    local now = os.date("*t")
    if now.year > year or
       (now.year == year and now.month > month) or
       (now.year == year and now.month == month and now.day > day) then

        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "เกมมีการอัปเดต",
            Text = "โปรดติดต่อผู้สร้าง",
            Icon = "rbxassetid://106671404092751",
            Duration = 10
        })

        if game.CoreGui:FindFirstChild("ESP_FOV_UI") then
            game.CoreGui["ESP_FOV_UI"]:Destroy()
        end
...
toggleImageBtn.Image = "rbxassetid://106671404092751"
